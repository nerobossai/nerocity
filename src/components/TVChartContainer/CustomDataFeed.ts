...
import {
    ...
    ResolveCallback,
  } from "@/public/static/charting_library/charting_library"; // Adjust the path to your library
  ...
  class CustomDataFeed {
    ...
    private getConfig() {
      return {
        ...
        supported_resolutions: ["1", "5", "15", "60", "1D", "1M", "3M"] as ResolutionString[], // Explicit cast
      };
    }
    ...
    private async fetchData(from: number, to: number) {
      ...
      const updatedData = apiData.map((d, i) => ({ close: d.close, high: d.high, low: d.low, open: d.open, time: d.timestamp, volume: d.volume}))

      const filteredData = updatedData.filter((d) => d.time >= from  && d.time <= to );
